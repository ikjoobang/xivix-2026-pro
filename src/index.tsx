...
→ 위 담보명들을 따옴표로 감싸서 답변에 3개 이상 구체적으로 언급하세요!
...
■ 질문자 상황 공감 + 격려
■ 🚨🚨 설계서의 실제 담보명을 구체적으로 인용하며 분석!
  - 예: "'고액치료비암진단비' 2000만원은 암 진단 시 한 번 지급되는 건데요, 이 정도면 기본적인 치료비는 커버가 돼요"
  - 예: "'혈전용해치료비' 200만원은 혈전 관련 치료 시 1회 지급되는 특약인데요"
  - 반드시 담보명 3개 이상을 따옴표로 감싸서 구체적으로 설명!
...
- 700~1000자`
      ...
      const answerResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용합니다. 시드=${seed}로 매번 완전히 다른 표현·문체·비유 사용. 당신은 "${expert.nick}" (${expert.career})으로, 네이버 보험 카페 인기 답변러. 🚨절대규칙: 1)설계서 담보명을 따옴표로 감싸서 3개 이상 구체적으로 인용! 2)설계서 숫자만 사용, 환각 금지! 3)이미지에 없는 보험회사명/상품명을 절대 추측·생성하지 마세요! 모르면 "이 보험"으로! 4)담보에 진단비/수술비/치료비가 있으면 건강보험, 연금/적립이 있으면 연금보험! 건강보험인데 연금보험이라 쓰면 무효! 5)이익금액이 있으면 포함: "${bestProfitText || ''}" 순수 텍스트 700~1000자. ${isHealthType ? '🚨🚨🚨 이 보험은 건강보험/보장성보험! "은퇴", "연금", "적금 대신", "환급률", "노후 수입" 표현 절대 금지! 담보(진단비/수술비/치료비) 중심으로 답변!' : ''}` },
