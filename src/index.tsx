...
JSON 형식으로만 응답 (마크다운 블록·설명 없이 순수 JSON만):
{
  ...
  "user_concern": "사용자의 핵심 고민 1~2문장 요약"
      ...
      if (ageNum > 0 && ageNum <= 18) {
        ...
        // 중장년
        writerPool = [
          { who: '40대 직장인', why: '가족 보장 강화', tone: '신중한 가장', start: '안녕하세요, 가족을 위해 보험을 알아보고 있습니다 🙋‍♂️' },
      ...
      const titlePatterns = [
        ...
        `\${company} \${product} 설계서 받았는데 월 \${premium} 적금 대신 넣어도 될까요?`,
        `\${product} \${period}년 넣으면 \${surrender}이라는데 진짜 적금보다 나은 건가요?`,
        `은행 적금 대신 \${product} 추천받았는데… 이 환급률 믿어도 되나요?`,
        `\${situation}인데 \${company} \${product} 가입해도 될까요? 설계서 봐주세요`,
        `월 \${premium}씩 \${period}년… \${product} 환급률 보고 고민 중입니다 어떤가요?`,
        `\${company} \${product} vs 은행 적금, \${period}년 후 어디가 더 유리할까요?`,
...
🚨🚨🚨 절대 규칙 (위반 시 전체 무효) 🚨🚨🚨
==================================================
1. 🔴 상품명 "${productName}"을 정확히 사용! 다른 이름으로 절대 바꾸지 말 것!
2. 🔴 회사명 "${company}"를 정확히 사용! "손해보험↔생명" 혼동 금지!
3. 🔴 설계서 숫자만 사용 (1원도 만들지 마세요. 환급률 소수점까지 정확히!)
4. 🚨 이익 금액 "${bestProfitText || '계산 필요'}" 반드시 본문에 포함! (해약환급금 - 총납입)
5. "~됩니다/~입니다" 금지 → "~인데요/~거든요/~더라고요" 
6. 답을 내리지 마세요 (물어보는 글!)
7. SEO 키워드: "${analysisData.product_type || '연금보험'}" 2~3회, "보험상담" 1회
8. ⚠️ 같은 입력이어도 시드(${seed})가 다르면 완전히 다른 글이 나와야 합니다
9. 피보험자가 ${ageNum <= 18 ? '미성년자(' + insuredAge + '세)이므로 "부모가 자녀를 위해 가입"하는 맥락으로 작성!' : '성인이므로 본인 가입 맥락으로 작성!'}
    ...
    "출처": "가입설계서"
      ...
      const qaResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용하는 서비스입니다. 시드=${seed}로 매번 완전히 다른 글을 만드세요. 🚨절대규칙: 1)상품명 "${productName}"을 한 글자도 바꾸지 마세요! 2)회사명 "${company}"를 정확히 사용! 3)숫자를 1원이라도 바꾸면 무효! 4)제목은 반드시 질문형("~까요?") 5)본문 800자 이상 6)🔴이익금액 "${bestProfitText}" 반드시 본문에 포함! 7)SEO/AEO/C-RANK/GEO 최적화. ${ageNum <= 18 ? '피보험자가 ' + insuredAge + '세 미성년자이므로 부모가 자녀를 위해 가입하는 맥락!' : ''}` },
...
- 80~120자, 원글에 없는 포인트 (비과세, 중도인출, 타사비교 등)
...
JSON: {"comments": [{"nickname": "${nick1}", "text": "..."}, {"nickname": "${nick2}", "text": "..."}, {"nickname": "${nick3}", "text": "..."}]}`
      ...
      const commentResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용합니다. 시드=${seed}로 매번 다른 내용·어휘·경험담을 만드세요. 네이버 카페 실제 회원 3명을 연기합니다. AI 느낌 절대 금지. 각 댓글은 성격·경험·말투가 확실히 달라야 합니다.` },
            { role: 'user', content: commentPrompt }
          ],
          max_tokens: 1200,
          temperature: 0.85
