...
JSON 형식으로만 응답 (마크다운·설명 없이):
{
  ...
  "special_notes": "설계서 특이사항·참고사항"
      ...
      // 글쓴이 페르소나 랜덤 풀 (매번 다른 시점/상황/화자)
      const writerPool = [
        { who: '30대 초보 맘', why: '아이 교육자금 마련', tone: '걱정 많은 엄마', start: '안녕하세요~ 보험 완전 초보 맘이에요 😅' },
        { who: '40대 직장인 아빠', why: '가족 보장 강화', tone: '신중한 가장', start: '안녕하세요, 직장인 아빠입니다 🙋‍♂️' },
        { who: '신혼부부 아내', why: '부부 노후 준비', tone: '꼼꼼한 신혼', start: '결혼하고 처음으로 보험 알아보는 중이에요 😊' },
        { who: '50대 자영업자', why: '노후 연금 대비', tone: '현실적 걱정', start: '자영업 하다 보니 노후가 갑자기 걱정되더라고요 😥' },
        { who: '20대 사회초년생', why: '첫 보험 가입', tone: '순수한 궁금증', start: '보험이란 걸 처음 알아보는 사회초년생이에요 🤔' },
        { who: '30대 프리랜서', why: '불안정한 소득 대비', tone: '불안한 미래', start: '프리랜서라 소득이 불안정해서 보험을 알아보게 됐어요' },
        { who: '두 아이 워킹맘', why: '자녀 미래 준비', tone: '바쁜 엄마', start: '아이 둘 키우면서 보험까지 신경 쓰자니 머리가 아프네요 😅' },
...
- ⚠️ 이익 금액 직접 계산! (해약환급금 - 총납입 = OOO만원 이익)
- 공시이율/최저보증이율도 언급
...
🔒 절대 규칙
==================================================
1. 설계서 숫자만 사용 (1원도 만들지 마세요)
2. 이익 금액 = 해약환급금 - 총납입 (이것만 계산 허용)
3. "~됩니다/~입니다" 금지 → "~인데요/~거든요/~더라고요" 
4. 답을 내리지 마세요 (물어보는 글!)
5. SEO 키워드: "${analysisData.product_type || '연금보험'}" 2~3회, "보험상담" 1회
6. ⚠️ 같은 입력이어도 시드(${seed})가 다르면 완전히 다른 글이 나와야 합니다
    ...
    "출처": "가입설계서"
      ...
      const qaResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용하는 서비스입니다. 매번 완전히 다른 글을 만들어야 합니다. 시드=${seed}를 반영해 어휘·문장구조·표현을 매번 다르게 하세요. 네이버 보험 카페 초보자 질문글을 작성하되: 1)제목은 반드시 질문형("~까요?") 2)본문 800자 이상 필수 3)이익금액(환급금-총납입) 계산 포함 4)설계서 숫자만 사용(할루시네이션 금지) 5)SEO/AEO/C-RANK/GEO 상위노출 최적화. 실제 카페 회원처럼 자연스럽게!` },
...
- 700~1000자`
      ...
      const answerResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용합니다. 시드=${seed} 기반으로 매번 완전히 다른 표현·문체·비유를 사용하세요. 당신은 "${expert.nick}" (${expert.career})으로, 네이버 보험 카페 인기 답변러입니다. 설계서 숫자만 사용하고, 단정적 표현 금지, SEO/C-RANK 키워드 자연 반복. 순수 텍스트로만 700~1000자 답변.` },
