...

{
  "comments": [
    ...
    const processedTitles = (expertData.titles || []).map((t: any) => ({
      ...t,
      text: t.text?.length > 40 ? t.text.substring(0, 37) + '...' : t.text,
      original_length: t.text?.length || 0,
      truncated: (t.text?.length || 0) > 40
    }))
...
✔️ 좋은 제목 예시 (각각 완전히 다른 5가지 패턴!):
- "타입A 해지고민: ${insuranceProduct} 해지하면 손해인가요? 진짜 고민돼요"
- "타입B 부모님대신: 부모님 ${insuranceProduct} 대신 내드리는데 이게 맞나요?"
- "타입C 갱신폭탄: ${insuranceProduct} 갱신료가 2배 뛰었는데 어쩌죠?"
- "타입D 결혼준비: 결혼 앞두고 ${insuranceProduct} 정리하라는데 맞아요?"
- "타입E 미루다가: ${insuranceProduct} 3개월째 미루는데 지금 가입해도 되나요?"
...
JSON 형식으로만 응답:
{
  "titles": [{"id":1,"text":"30~40자 고객관점 제목 (타입A 해지고민)"},{"id":2,"text":"30~40자 고객관점 제목 (타입B 부모님대신)"},{"id":3,"text":"30~40자 고객관점 제목 (타입C 갱신폭탄)"},{"id":4,"text":"30~40자 고객관점 제목 (타입D 결혼준비)"},{"id":5,"text":"30~40자 고객관점 제목 (타입E 미루다가)"}],
