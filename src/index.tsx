...
type Bindings = {
  ...
  OPENAI_API_KEY?: string;  // V2026.37.101 - OpenAI GPT-4o 지원
...
const ENGINE = {
  ...
  GPT4O: 'gpt-4o'                  // V2026.37.101 - OpenAI GPT-4o
}

// V2026.37.101 - OpenAI API 키는 환경변수에서 가져옴
...
// ============================================
// V2026.37.101 - OpenAI GPT-4o API 호출 함수
...
// V2026.37.101 - GPT용 시스템 프롬프트 (보험 전문가)
...
function analyzeTarget(topic: string, ocrData?: any) {
  // V2026.37.101 - 보험 종류 자동 감지 (건강보험 추가, 우선순위 조정)
...
// ✅ V2026.37.101 - 댓글 길이 다양화 (CEO 지시: 짧은것/긴것 섞어서)
...
// ✅ V2026.37.101 - 실시간 시세 API (금, 비트코인, 환율 - 백엔드에서 모두 처리)
app.get('/api/market-data', async (c) => {
  try {
    ...
    // 🔥 V2026.37.101 - 실시간 변동 시뮬레이션 (매초 변동)
...
app.post('/api/generate/full-package', async (c) => {
  try {
    ...
    if (image) {
      contextSource = 'image'
      // V2026.37.101 - OCR 프롬프트 완전 재설계 (담보 항목 기반 보험 종류 판단)
...
${parsed.advice || ''}`
          ...
          // V2026.37.101 - 이미지에서 추출한 모든 핵심 정보 저장
    ...
    // ============================================
    // 🚨 핵심 제약 조건 (프롬프트 최상단 배치) - V2026.37.101
    // ============================================
  ...
  "comments": [
    ...
    return c.json({
      ...
      package: {
        ...
        // V2026.37.101 - CEO 지시: 보험사명 추가 (자동 이미지 생성용)
...
app.post('/api/generate/full-package-stream', async (c) => {
  ...
  // V2026.37.101 - AI 모델 선택 (gemini | gpt)
  const aiModel = body.aiModel || 'gpt' // V2026.37.101 - GPT-4o 기본값
  ...
  return streamText(c, async (stream) => {
    try {
      ...
      // V2026.37.101 - 이미지에서 추출한 보험사/상품명 (자동 이미지 생성용)
...
${parsed.summary || ''}`
            ...
            // OCR 결과 스트림 전송 (V2026.37.101 - 추가 필드)
      ...
      // ✅ V2026.37.101 - OCR 데이터 강화 바인딩 (종신/100세만기 구분 추가)
      ...
      if (reportData.length > 0 || imageAnalysis) {
        ...
        // V2026.37.101 - 보험기간 표시 (종신 vs 100세만기 명확히!)
...
`
      ...
      // V2026.37.101 - 보험기간 정보 포함
      const periodText = ocrInsurancePeriod ? `(${ocrInsurancePeriod})` : ''
      
      const titlePrompt = `## XIVIX V2026.37.101 제목 + 바이럴 질문 생성 ##
${ocrContextBlock}
사용자 원본 입력: ${topic}
보험: ${insuranceProduct} ${periodText}
타겟: ${targetAudience}

🚨🚨🚨 [CEO 최우선 지시 - V2026.37.101] 🚨🚨🚨
"제목은 설계사가 아니라 고객이 짓는 거다"
...
📌 [제목 5개 생성 규칙 - V2026.37.101 OCR 반영 강화]
■ 공백 포함 20~25자 필수!
...
📌 [바이럴 질문 3개 생성 규칙 - V2026.37.101 강화]
...
JSON 형식으로만 응답:
{
  ...
  "viral_questions": [{"id":1,"text":"OCR데이터+${insuranceProduct}3회반복+질문형 150~250자"},{"id":2,"text":"OCR데이터+${insuranceProduct}3회반복+질문형 250~350자"},{"id":3,"text":"OCR데이터+${insuranceProduct}3회반복+질문형 350~450자"}]
      ...
      // ============================================
      // V2026.37.101 - GPT/Gemini 분기 처리 (GPT 프롬프트 대폭 강화)
      // ============================================
      if (useGPT) {
        // GPT-4o 사용
        ...
        // V2026.37.101 - GPT-4o 시스템 프롬프트 대폭 강화
...
JSON 형식으로만 응답하세요.`
      ...
      // V2026.37.101 - 이미지 OCR 데이터가 있으면 본문에 강제 바인딩
      ...
      // V2026.37.101 - imageAnalysis가 있으면 추가 바인딩 (보험사명, 상품명, 보험료, 보험기간 등)
      if (imageAnalysis) {
...
4. OCR에 없는 정보를 추측해서 쓰지 마세요!`
      ...
      // 🔥 V2026.37.101 - 이미지 OCR 성공 시 사용자 텍스트 입력 무시
...
{"text": "${lengthMode.min}~${lengthMode.max}자의 핵심 위주 답변"}`
        ...
        // ============================================
        // V2026.37.101 - 본문 생성 GPT/Gemini 분기
        // ============================================
        if (useGPT) {
          // GPT-4o로 본문 생성
          const openaiKey = c.env.OPENAI_API_KEY || OPENAI_API_KEY
...
{"comments":[{"id":1,"nickname":"닉네임","persona":"역할","text":"댓글"}]}`
      ...
      // ============================================
      // V2026.37.101 - 댓글 생성 GPT/Gemini 분기
      // ============================================
      if (useGPT) {
        const openaiKey = c.env.OPENAI_API_KEY || OPENAI_API_KEY
      ...
      // ============================================
      // 🔥 V2026.37.101 - CEO 긴급 지시: SEO 키워드 다양성 + 고정 키워드
...
// ✅ V2026.37.101 - PWA manifest.json
...
// ============================================
// V2026.37.101 - XIIM 프록시 API (도메인 제한 우회)
...
// ============================================
// V2026.37.101 - 한국어 보험 가입설계서 이미지 생성 API
...
app.post('/api/xiim/openai/generate', async (c) => {
  try {
    ...
    console.log('[XIVIX] V2026.37.101 한국어 설계서 생성:', company, insuranceType);
  ...
  <text x="300" y="770" font-family="Arial,sans-serif" font-size="10" fill="#94a3b8" text-anchor="middle">문의: ${company} 고객센터</text>
    ...
    console.log('[XIVIX] V2026.37.101 SVG 설계서 생성 완료:', company, insuranceType, monthlyPremium);
...
🔴🔴🔴 [빨간색/강조 표시 인식 - 매우 중요!] 🔴🔴🔴
■ 빨간색 박스, 빨간색 밑줄, 빨간색 동그라미, 형광펜 표시된 항목을 우선 인식!
■ 사용자가 빨간펜/형광펜으로 강조한 부분 = 사용자가 가장 궁금한 부분!
■ 강조된 담보/금액은 key_benefits 맨 앞에 "★" 표시하여 배치
■ special_notes에 "빨간색으로 강조된 항목: XXX" 형태로 기록
■ 손글씨/메모가 있으면 special_notes에 기록
...
JSON만 응답:
{
  ...
  "special_notes": "빨간색 강조 항목, 손글씨 메모 등",
...
JSON만 응답:
{
  ...
  "user_concern": ""
    ...
    const questionsPrompt = `[🔒🔒🔒 당신의 정체: 보험을 1도 모르는 완전 초보자]
당신은 보험에 대해 정말 아무것도 모릅니다.
- "진단비"가 뭔지도 모릅니다
- "특약"이란 단어를 처음 들어봅니다
- 설계서를 받았는데 표가 복잡하고 글씨가 작아서 뭐가 뭔지 하나도 모릅니다
- 그래서 네이버 카페에 "이게 뭐예요?" 하고 물어보려 합니다

⚠️ 당신은 절대 보험 전문가가 아닙니다! 전문가처럼 분석하는 질문은 금지!
...
• 특약/담보 목록: ${benefits || '확인 필요'}
• 공시이율: ${analysisData.interest_rate || '?'}

${newsText ? `[사용자가 추가로 적은 텍스트 (힌트)]
"${newsText}"
→ 이 힌트를 반영한 질문을 1~2개 우선 배치!
` : ''}

[🔴🔴🔴 초보자 질문 = 이런 수준입니다! 반드시 따라하세요!]

✅ 좋은 예시 (진짜 초보자):
- "설계서에 '119대 질병'이라고 적혀있는데 이게 뭐예요? 119개나 되는 건가요?"
- "여기 '상해통원' 뭐라고 써있는데 이게 병원 가면 돈 주는 건가요?"
- "설계사가 '진단비'가 중요하다는데 진단비가 뭐예요? 진단 받으면 돈을 주는 건가요?"
- "'뇌혈관질환진단비'라고 2000만원 적혀있는데... 뇌혈관이 뭔가요? 뇌졸중이랑 같은 건가요?"
- "특약이 이렇게 많은데 이거 다 돈 내는 건가요? 빼면 보험료 줄어드나요?"
- "월 5만원이면 비싼 건가요 싼 건가요? 보험을 처음 들어봐서 감이 안 잡혀요"
- "20년 넣으라는데... 20년이면 너무 긴 거 아닌가요? 중간에 못 내면 어떻게 되나요?"
- "'고액치료비암진단비'라고 적혀있는데 이게 암 걸리면 돈 주는 건가요?"
- "설계서에 빨간색으로 표시된 부분이 있는데 이게 특별히 중요한 건가요?"
- "이 보험 가입하면 아프면 바로 돈 나오는 건가요? 조건이 있나요?"

❌ 나쁜 예시 (전문가 질문 - 이렇게 쓰면 안 됨!):
- "고액치료비암진단비의 보장범위가 어떻게 되나요?" (← 초보자는 '보장범위'란 말을 모름)
- "이 상품의 납입면제 조건이 궁금합니다" (← 초보자는 '납입면제'를 모름)
- "갱신형인지 비갱신형인지 확인하고 싶습니다" (← 초보자는 '갱신/비갱신' 개념이 없음)
- "해약환급률 대비 실질수익률은?" (← 완전 전문가 질문)
- "이 특약의 보장 한도와 면책기간은?" (← 초보자는 '면책기간'을 모름)

[🔴 핵심 규칙]
1. 설계서에 있는 실제 담보명/특약명을 따옴표로 인용하되, "이게 뭐예요?" "이게 뭔 말이에요?" 수준으로 질문
2. 전문 용어(보장범위, 납입면제, 갱신형, 면책기간, 실질수익률 등)를 질문에 사용하지 마세요!
3. 초보자가 쓸 법한 표현만: "~뭐예요?", "~인가요?", "~건가요?", "~되나요?", "~몰라서요", "~처음이라"
4. 각 질문에 설계서의 실제 담보명이나 금액을 1개 이상 인용 (따옴표로 감싸기)
5. 🚨 이미지에 없는 보험회사명/상품명 절대 추측 금지! 모르면 "이 보험"으로!

[질문 배분]
- 4~5개: 실제 담보명을 인용 + "이게 뭐예요?" / "이게 필요한 건가요?" 
- 2~3개: 보험료/기간 관련 초보 질문 ("비싼 건가요?", "오래 내야 하나요?")
- 1~2개: 가입 판단 ("이거 가입해도 되는 건가요?", "이거 좋은 건가요?")
- 1개: 사용자 힌트 반영 (있을 경우)

JSON만 응답:
{
  "questions": [
    {"id": 1, "text": "초보자 질문", "category": "특약", "related_data": "관련 데이터"},
...
10개 정확히 생성하세요.`
    ...
    const questionsResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: '🚨🚨🚨 당신은 보험을 1도 모르는 완전 초보자입니다. "진단비"가 뭔지도 모르고, "특약"이란 단어를 처음 봅니다. 전문 용어(보장범위, 납입면제, 갱신형, 면책기간, 보장한도, 실질수익률)를 질문에 절대 사용하지 마세요! "이게 뭐예요?", "이게 뭔 말이에요?", "이게 필요한 건가요?" 수준으로만 질문하세요. 카페에서 물어보듯이 질문합니다. 전문가처럼 분석하는 질문은 전부 탈락!' },
...
app.post('/api/generate/news-qa', async (c) => {
  ...
  let imageArray: Array<{base64: string, mimeType: string}> = []
  if (images.length > 0) {
    imageArray = images.slice(0, 10) // 최대 10장
  } else if (image) {
    imageArray = [{ base64: image, mimeType: body.mimeType || 'image/jpeg' }]
  }
  
  // V2026.37.110 - selectedQuestion + analysisCache 모드에서는 이미지/텍스트 없이도 진행 가능
  ...
  return streamText(c, async (stream) => {
    try {
      ...
      if (hasBoth) {
        // ★★★ 텍스트+이미지 동시 모드 ★★★
        // 텍스트 = 사용자의 질문/요구사항 (우선순위 1)
        // 이미지 = OCR로 상품 데이터 추출 (우선순위 2, 보조 근거)
        analysisPrompt = `[시스템 역할]
당신은 대한민국 보험업계 30년 경력 MDRT 종신회원이자 보험계리사입니다.
사용자가 보험 설계서 이미지와 함께 질문을 보냈습니다.

🔴🔴🔴 [최우선 규칙] 🔴🔴🔴
1. 사용자 텍스트 = "질문/요구사항" → 이것이 답변의 출발점!
2. 이미지 = "설계서/증권" → OCR로 데이터 추출하여 질문의 근거 자료로 활용
3. 답변은 반드시 사용자 질문에 맞춰서 생성!

[사용자 질문 (최우선)]
${newsText}

[분석 지침]
1. 이미지에서 OCR로 보험 상품 데이터를 정확히 추출
2. 추출한 데이터를 기반으로 사용자 질문에 직접 답변할 수 있는 정보 구성
3. user_concern에 사용자의 원문 질문을 그대로 반영
...
- 정보 없으면 빈 문자열("") 반환
- 해약환급금 테이블: 가능한 모든 행 추출
- 빨간 박스/강조 표시된 담보는 special_notes에 "빨간박스: [내용]" 기록

🚨🚨 [담보/특약 추출 핵심] 🚨🚨
- key_benefits에 설계서의 모든 담보명을 "이미지 그대로 + (금액)" 형식으로 추출!
- 빨간 박스 담보는 "★" 표시하여 맨 앞 배치
- 담보 기반으로 보험종류 자동 판별 (진단비/수술비 → 건강보험, 연금/적립 → 연금보험)

JSON 형식으로만 응답:
{
  "company": "보험회사명 (이미지에서 확인, 안 보이면 빈 문자열)",
  "product_name": "상품명 (이미지에서 확인, 안 보이면 빈 문자열)",
  "product_type": "담보 기반 자동판별: 건강보험/연금보험/종신보험 등",
  "insured_age": "피보험자 나이",
  "insured_gender": "성별",
  ...
  "key_benefits": ["★빨간박스담보(금액)", "담보명1(금액)", "담보명2(금액)", "모든 담보를 이미지 그대로 추출"],
  "special_notes": "빨간 박스 표시 항목, 설계서 특이사항",
  "user_concern": "${newsText}"
}`
      } else if (hasOnlyText) {
...
JSON 형식으로만 응답 (설명 없이):
{
  ...
  "user_concern": "사용자의 핵심 고민 요약"
        ...
        analysisPrompt = `[시스템 역할]
당신은 보험 가입설계서/보험증권 이미지에서 데이터를 추출하는 OCR 전문 시스템입니다.
당신의 유일한 역할은 이미지에 인쇄된 텍스트와 숫자를 "있는 그대로" 읽어서 JSON으로 출력하는 것입니다.

${newsText ? '[사용자 추가 입력 텍스트]\\n' + newsText + '\\n\\n' : ''}

🚨🚨🚨 절대 위반 금지 규칙 (1개라도 위반 시 전체 무효) 🚨🚨🚨

1. 상품명: 이미지에 인쇄된 상품명을 "한 글자도 바꾸지 말고" 그대로 출력
   - 이미지에서 상품명이 보이지 않으면 → 빈 문자열("") 반환! 절대 추측 금지!
   
2. 회사명: 이미지 상단 로고/CI/워터마크에서 확인
   - 🚨 이미지에서 회사명이 명확히 보이지 않으면 → 빈 문자열("") 반환! 
   - 🚨 "삼성생명", "한화생명" 등 추측·생성 절대 금지!
   - 보이는 텍스트가 확실할 때만 기재
...
4. 이미지에 없는 정보: 빈 문자열("") 반환. 절대 추측·생성 금지

5. 해약환급금 테이블 추출 규칙:
   - "최저보증이율 가정시" 컬럼을 기본으로 추출 (보수적 수치)
   - 주요 시점: 3년, 5년, 7년, 10년, 15년, 20년, 25년, 30년 등 가능한 모든 행
   - 빨간 박스/강조 표시된 행은 반드시 포함하고 special_notes에 "빨간박스: [내용]" 기록

6. 피보험자 정보:
   - 주민등록번호 앞자리로 나이 계산 (예: 160116 → 2016년생)
   - 주민번호 뒷자리 첫째 숫자로 성별 (1,3=남, 2,4=여)

🚨🚨🚨 [핵심: 담보/특약 목록 추출 - key_benefits] 🚨🚨🚨
7. key_benefits에는 설계서에 보이는 모든 담보명/특약명을 "이미지 표기 그대로" 추출!
   - 각 담보의 보장금액도 괄호 안에 포함 (예: "고액치료비암진단비(2,000만원)")
   - 담보명을 축약하거나 바꾸지 말 것! 이미지에 쓰인 그대로!
   - 빨간 박스/강조 표시된 담보는 맨 앞에 배치하고 "★" 표시
   - 예: ["★혈전용해치료비(200만원)", "고액치료비암진단비(2,000만원)", "뇌혈관질환진단비(2,000만원)"]

8. 보험종류(product_type) 자동 판별:
   - 담보가 진단비/수술비/치료비/입원비 위주 → "건강보험" 또는 "보장성보험"
   - 담보가 연금/적립/환급금 위주 → "연금보험" 또는 "저축성보험"  
   - 담보가 사망보장 위주 → "종신보험"
   - 🚨 건강보험 담보(진단비/수술비)인데 "연금보험"이라고 쓰면 전체 무효!

JSON 형식으로만 응답 (마크다운 블록·설명 없이 순수 JSON만):
{
  "company": "이미지에서 확인된 보험회사명 (보이지 않으면 빈 문자열)",
  "product_name": "이미지에 인쇄된 상품명 그대로 (보이지 않으면 빈 문자열)",
  "product_type": "건강보험/연금보험/종신보험/암보험/어린이보험/교육보험/변액보험/저축보험 (담보 기반 자동 판별)",
  "insured_age": "피보험자 나이 (숫자만, 예: 44)",
  "insured_gender": "남/여",
  "payment_period": "납입기간 숫자만 (예: 20)",
  "coverage_period": "보험기간 (예: 90세만기)",
  "monthly_premium": "월보험료 (이미지에서 확인, 없으면 빈 문자열)",
  "total_premium": "총납입금액 (이미지에서 확인, 없으면 빈 문자열)",
  "insurance_amount": "보험가입금액 (이미지에서 확인, 없으면 빈 문자열)",
  "surrender_values": [
    {"year": 5, "amount": "이미지 표기 그대로", "rate": "42.4%"},
    {"year": 10, "amount": "이미지 표기 그대로", "rate": "130.0%"}
  ],
  "interest_rate": "공시이율 X.X% / 최저보증이율 X.X% (이미지에 표기된 그대로, 없으면 빈 문자열)",
  "key_benefits": ["★빨간박스담보명(금액)", "담보명1(금액)", "담보명2(금액)", "이미지에서 보이는 모든 담보"],
  "special_notes": "빨간 박스 표시 항목, 설계서 특이사항",
  "user_concern": "사용자의 핵심 고민 1~2문장 요약"
}`
      }
      ...
      if (hasOnlyText) {
        ...
        analysisResponse = await fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            model: 'gpt-4o',
            messages: [
              { role: 'system', content: '보험 설계서 OCR 전문 시스템. 🔴최우선: 빨간색 박스/밑줄/동그라미/형광펜으로 강조된 항목을 반드시 인식하고 key_benefits 맨 앞에 ★표시로 배치. 사용자가 강조한 부분이 가장 궁금한 부분입니다. 금액 정확도: 2,000만원 vs 200만원 쉼표 반드시 확인. 이미지에 없는 회사명/상품명 추측 절대 금지.' },
              { role: 'user', content: contentParts }
            ],
            max_tokens: 4096,
            temperature: 0.2
      ...
      if (analysisResponse.ok) {
        ...
        try {
          ...
          console.error('[XIVIX] 분석 JSON 파싱 실패:', e)
          analysisData = { raw_text: rawText }
        }
      } else {
      ...
      console.log('[XIVIX] 설계서 분석 완료:', analysisData.product_name)
      
      // ============================================
      // Step 2: 분석 결과 기반 Q&A 콘텐츠 생성 (GPT-4o)
      // ============================================
      await stream.write(JSON.stringify({ type: 'step', step: 2, msg: '✍️ 보험 카페 Q&A 콘텐츠 생성 중...' }) + '\n')
      
      // 핵심 정보 추출
      const productName = analysisData.product_name || '보험상품'
      const company = analysisData.company || '보험사'
      const insuredAge = analysisData.insured_age || ''
      ...
      // 해약환급금 정보 포맷팅
      let surrenderInfo = ''
      let bestProfitText = '' // ✅ V2026.37.103 - 이익 금액 서버에서 직접 계산
      if (analysisData.surrender_values && analysisData.surrender_values.length > 0) {
        surrenderInfo = analysisData.surrender_values
          .map((sv: any) => `${sv.year}년: ${sv.amount} (환급률 ${sv.rate})`)
          .join(', ')
        
        // ✅ 이익 금액 자동 계산 (GPT에게 맡기지 않고 서버에서 직접!)
        const totalNum = parseInt(String(totalPremium).replace(/[^0-9]/g, '')) || 0
        if (totalNum > 0) {
          let bestProfit = 0
          let bestYear = ''
          let bestAmount = ''
          for (const sv of analysisData.surrender_values) {
            const svNum = parseInt(String(sv.amount).replace(/[^0-9]/g, '')) || 0
            const profit = svNum - totalNum
            if (profit > bestProfit) {
              bestProfit = profit
              bestYear = sv.year
              bestAmount = sv.amount
            }
          }
          if (bestProfit > 0) {
            // 만원 단위로 변환
            const profitMan = Math.round(bestProfit / 10000)
            const totalMan = Math.round(totalNum / 10000)
            bestProfitText = `${bestYear}년 시점 해약환급금 ${bestAmount} - 총납입 ${totalMan}만원 = 약 ${profitMan}만원 이익`
          }
        }
      }
      
      // ✅ V2026.37.103 - 중복 방지 랜덤 시드 시스템
      // 수천 명의 보험설계사가 사용 → 매번 완전히 다른 콘텐츠 필수
      const now = Date.now()
      const seed = now % 1000000
      
      // ✅ V2026.37.103 - 피보험자 나이 기반 페르소나 자동 선택
      // 10세 아이 보험인데 "은퇴 준비" 페르소나가 나오면 안 됨!
      const ageNum = parseInt(insuredAge) || 0
      
      let writerPool: Array<{who: string, why: string, tone: string, start: string}> = []
      
      // V2026.37.107: 보험종류 사전 판별 (페르소나 선택 전)
      const isHealthForPersona = /건강|보장|진단|수술|치료|입원/.test(analysisData.product_type || '') || /진단비|수술비|치료비/.test(benefits)
      
      if (ageNum > 0 && ageNum <= 18) {
        // 미성년자(아이) 보험 → 부모 페르소나 (보험종류에 따라 분기)
        if (isHealthForPersona) {
          writerPool = [
            { who: '30대 초보 맘', why: `${ageNum}살 아이가 아파서 보험 점검`, tone: '걱정 많은 엄마', start: `안녕하세요~ ${ageNum}살 아이 키우는 초보 맘이에요. 아이가 자주 아파서 보험을 점검하게 됐어요 😅` },
            { who: '40대 직장인 아빠', why: `${ageNum}세 자녀 건강보험 보장 확인`, tone: '신중한 가장', start: `안녕하세요, ${ageNum}살 아이 아빠입니다. 아이 건강보험 보장이 충분한지 확인하려고요 🙋‍♂️` },
            { who: '두 아이 워킹맘', why: `아이들 건강 보장 챙기기`, tone: '바쁜 엄마', start: `아이 키우면서 건강보험 보장이 충분한지 궁금해서 알아보고 있어요 😅` },
          ]
        } else {
          writerPool = [
            { who: '30대 초보 맘', why: `${ageNum}살 아이 목돈 마련`, tone: '걱정 많은 엄마', start: `안녕하세요~ ${ageNum}살 아이 키우는 초보 맘이에요 😅` },
            { who: '40대 직장인 아빠', why: `${ageNum}세 자녀 교육자금 준비`, tone: '신중한 가장', start: `안녕하세요, ${ageNum}살 아이 아빠입니다 🙋‍♂️` },
            { who: '두 아이 워킹맘', why: `아이들 미래 준비`, tone: '바쁜 엄마', start: `아이 키우면서 적금 대신 보험으로 목돈 만들 수 있다길래 알아보고 있어요 😅` },
            { who: '30대 맞벌이 부부', why: `${ageNum}세 자녀 적금 대안 찾기`, tone: '꼼꼼한 부모', start: `은행 적금 이자가 너무 낮아서 아이 앞으로 연금보험을 알아보게 됐어요` },
            { who: '30대 전업맘', why: `${ageNum}살 아이 교육비 마련`, tone: '알뜰한 엄마', start: `아이가 점점 크니까 교육자금이 걱정되기 시작하더라고요 🤔` },
            { who: '40대 맞벌이 아빠', why: `자녀 미래 자금 설계`, tone: '계획적 아빠', start: `아이 앞으로 매달 조금씩 넣어두면 나중에 도움이 될까 싶어 보험을 알아보고 있어요` },
          ]
        }
      } else if (ageNum >= 19 && ageNum <= 35) {
        // 청년~사회초년생 (보험종류에 따라 분기)
        if (isHealthForPersona) {
          writerPool = [
            { who: '20대 사회초년생', why: '첫 건강보험 가입 고민', tone: '순수한 궁금증', start: '건강보험이란 걸 처음 알아보는 사회초년생이에요 🤔' },
            { who: '신혼부부 아내', why: '부부 건강 보장 시작', tone: '꼼꼼한 신혼', start: '결혼하고 건강보험을 처음 알아보는 중이에요 😊' },
            { who: '30대 프리랜서', why: '건강 위험 대비', tone: '불안한 미래', start: '프리랜서라 아프면 소득이 끊기니까 건강보험을 알아보게 됐어요' },
          ]
        } else {
          writerPool = [
            { who: '20대 사회초년생', why: '첫 보험 가입 고민', tone: '순수한 궁금증', start: '보험이란 걸 처음 알아보는 사회초년생이에요 🤔' },
            { who: '신혼부부 아내', why: '부부 노후 준비 시작', tone: '꼼꼼한 신혼', start: '결혼하고 처음으로 보험 알아보는 중이에요 😊' },
            { who: '30대 프리랜서', why: '불안정한 소득 대비', tone: '불안한 미래', start: '프리랜서라 소득이 불안정해서 보험을 알아보게 됐어요' },
          ]
        }
      } else if (ageNum >= 36 && ageNum <= 55) {
        // 중장년 - V2026.37.106: 보험종류에 따라 페르소나 분기
        const isHealthInsurance = /건강|보장|진단|수술|치료|입원/.test(analysisData.product_type || '') || /진단비|수술비|치료비/.test(benefits)
        if (isHealthInsurance) {
          writerPool = [
            { who: '40대 직장인', why: '건강검진에서 소견 나와서 보험 점검', tone: '걱정되는 가장', start: '안녕하세요, 최근 건강검진에서 주의 소견이 나와서 보험을 다시 점검하게 됐어요 😥' },
            { who: '44세 사무직', why: '가족력 때문에 암/뇌/심장 보장 강화', tone: '신중한 가장', start: '부모님 중 한 분이 뇌혈관 쪽으로 수술을 하셔서 저도 보장을 챙겨야겠다 싶었어요' },
            { who: '40대 맞벌이', why: '보험 리모델링 상담 받고 설계서 확인 중', tone: '꼼꼼한 성격', start: '보험 리모델링 상담을 받았는데 설계서를 보고 고민이 생겼어요 🤔' },
          ]
        } else {
          writerPool = [
            { who: '40대 직장인', why: '가족 노후 준비', tone: '신중한 가장', start: '안녕하세요, 가족을 위해 보험을 알아보고 있습니다 🙋‍♂️' },
            { who: '50대 자영업자', why: '노후 연금 대비', tone: '현실적 걱정', start: '자영업 하다 보니 노후가 갑자기 걱정되더라고요 😥' },
            { who: '40대 맞벌이 부부', why: '은퇴 후 안정적 수입', tone: '실용적', start: '은퇴 후를 생각하면 연금 준비가 시급하더라고요' },
          ]
        }
      } else {
        // 나이 불명 또는 56세 이상 - V2026.37.106: 보험종류에 따라 분기
        const isHealthInsurance = /건강|보장|진단|수술|치료|입원/.test(analysisData.product_type || '') || /진단비|수술비|치료비/.test(benefits)
        if (isHealthInsurance) {
          writerPool = [
            { who: '보험 점검 중', why: '기존 보험 보장이 부족한지 확인', tone: '걱정되는', start: '보험을 가입한 지 오래 됐는데 보장이 충분한지 궁금해서 점검 중이에요 🤔' },
            { who: '건강 걱정 중', why: '주변에서 큰 병 얘기 듣고 보장 확인', tone: '불안한', start: '주변 지인이 큰 수술을 해서 저도 보장이 되는지 확인하려고요 😥' },
            { who: '보험 초보', why: '처음으로 설계서 분석', tone: '알고 싶은', start: '보험을 처음 알아보는데 설계서를 받고 고민이 많아요 🤔' },
          ]
        } else {
          writerPool = [
            { who: '은퇴 준비 중인 부부', why: '연금 수령 계획', tone: '진지한 고민', start: '은퇴가 얼마 남지 않아서 연금보험을 진지하게 보고 있어요' },
            { who: '50대 직장인', why: '노후 대비', tone: '현실적', start: '정년이 다가오니 연금 준비가 급해졌어요' },
            { who: '보험 초보', why: '보험 비교 분석', tone: '알고 싶은', start: '보험을 처음 알아보는데 설계서를 받고 고민이 많아요 🤔' },
          ]
        }
      }
      const writer = writerPool[seed % writerPool.length]
      
      // 제목 패턴 랜덤 풀 (항상 질문형!) - V2026.37.106: 보험종류에 따라 분기
      const isHealthType = /건강|보장|진단|수술|치료|입원/.test(analysisData.product_type || '') || /진단비|수술비|치료비/.test(benefits)
      let titlePatterns: string[] = []
      if (isHealthType) {
        titlePatterns = [
          `\${situation}… 설계서 받았는데 이 담보들 다 필요한 건가요?`,
          `\${product} 설계서 봐주세요~ 담보 항목이 많은데 꼭 필요한 건지 모르겠어요`,
          `\${period}년납 건강보험인데 이 보장 금액이면 충분한가요?`,
          `\${situation}인데 진단비/수술비 보장이 이 정도면 괜찮을까요?`,
          `건강보험 설계서 받았는데 빼도 되는 특약이 있나요?`,
          `\${situation} 때문에 건강보험 알아보는데 이 설계서 조건 어떤가요?`,
          `암/뇌/심장 보장이 이 금액이면 충분할까요? 설계서 봐주세요`,
          `\${product} 담보 구성 괜찮은지 전문가분들 조언 부탁드려요`,
        ]
      } else {
        titlePatterns = [
          `\${situation}… \${product} \${period}년 납입인데 환급률 이 정도면 괜찮을까요?`,
          `\${product} 설계서 받았는데 월 \${premium} 적금 대신 넣어도 될까요?`,
          `\${product} \${period}년 넣으면 \${surrender}이라는데 진짜 적금보다 나은 건가요?`,
          `은행 적금 대신 \${product} 추천받았는데… 이 환급률 믿어도 되나요?`,
          `\${situation}인데 \${product} 가입해도 될까요? 설계서 봐주세요`,
          `월 \${premium}씩 \${period}년… \${product} 환급률 보고 고민 중입니다 어떤가요?`,
          `\${product} vs 은행 적금, \${period}년 후 어디가 더 유리할까요?`,
          `\${situation} 때문에 \${product} 알아보는데 이 설계서 조건이 괜찮은 건지요?`,
        ]
      }
      const selectedTitlePattern = seed % titlePatterns.length
      
      // 구조 변형 (매번 다른 순서/비중) - V2026.37.106: 보험종류에 따라 분기
      let structureVariants: string[] = []
      if (isHealthType) {
        structureVariants = [
          '상황→설계서담보→보장고민→질문→근거',
          '설계서담보→상황→특약고민→질문→근거',
      ...
      const selectedStructure = structureVariants[seed % structureVariants.length]

      const qaPrompt = `[🔒 핵심 지침: 이 서비스는 수천 명의 보험설계사가 사용합니다. 콘텐츠가 절대 중복되면 안 됩니다!]

랜덤시드: ${seed} (이 숫자를 기반으로 표현·어휘·문장구조를 매번 완전히 다르게 만드세요)
...
${selectedQuestion ? `(사용자가 10가지 질문 중 선택한 초보자 질문입니다!)
🚨🚨🚨 초보자 질문 모드 규칙:
- 제목: 이 질문을 그대로 또는 살짝 다듬어서 제목으로!
- 본문: 초보자가 쓴 것처럼 "~거든요", "~인데요", "~모르겠어요" 어투 유지
- 전문 용어 설명 요구 금지! "이게 뭐예요?" "이게 필요한 건가요?" 수준 유지
- 설계서 담보명은 따옴표로 감싸되, 설명 없이 그냥 "이런 게 적혀있더라고요" 식으로` : ''}
...
• 상품명: ${productName} ← 이 상품명을 정확히 인용! (다른 이름으로 절대 변경 금지)
• 상품종류: ${analysisData.product_type || '보험상품'}
• 피보험자: ${insuredAge ? insuredAge + '세' : ''} ${analysisData.insured_gender || ''} ${ageNum <= 18 ? '(미성년자 - 보호자가 가입)' : ''}
• 납입기간: ${paymentPeriod}
• 보험기간: ${analysisData.coverage_period || '확인 필요'}
• 월보험료: ${monthlyPremium}
• 총납입금액: ${totalPremium}
...
- 담보에 "연금", "적립", "환급금", "해약환급" 등이 있으면 → "연금보험/저축성보험"
- 🚨 건강보험인데 "연금보험"이라고 쓰면 전체 무효!
- 🚨 이미지에 없는 보험회사명/상품명을 절대 추측·생성하지 마세요!

==================================================
📌 제목 (반드시 질문형! 네이버 SEO/AEO/C-RANK/GEO 1위 목표)
==================================================
■ 길이: 30~50자
■ ⚠️ 반드시 "~까요?", "~인가요?", "~될까요?", "~나요?" 등 질문으로 끝나야 함
■ 패턴 참고 (${selectedTitlePattern}번 기반으로 변형): ${titlePatterns[selectedTitlePattern]}
■ 🚨 제목에 설계서의 실제 담보명을 1개 이상 포함! (예: "혈전용해치료비", "뇌혈관질환진단비")
■ 필수 포함: 실제 담보명 또는 보험종류(${analysisData.product_type || '보험'})
■ 사용자 상황(${writer.why}) 자연스럽게 반영
■ 금지: 가이드, 전략, 충격, 필독, 대박, 절대, 총정리
■ 🚨 이미지에 없는 보험회사명은 절대 사용 금지! 모르면 "보험"만 사용!
■ SEO 핵심: 네이버 사용자가 실제로 검색할 질문 + 핵심 숫자(보장금액 등)

==================================================
📝 본문 (800~1200자, 구조: ${selectedStructure})
==================================================

⚠️ 800자 미만 절대 금지! 각 단계를 충분히 풍부하게!

【상황 설명 (4~5줄)】
- "${writer.start}"로 시작 (이 문장 그대로 사용!)
- ${writer.why} 때문에 보험을 알아보게 된 경위를 자연스럽게
- 구어체: "~거든요", "~인데요", "~더라고요"

【설계서 담보/특약 공유 (6~8줄) - 🚨핵심🚨】  
- 🚨🚨 설계서에 있는 실제 담보명을 구체적으로 나열하며 공유!
- 예시: "설계서를 보니까 '고액치료비암진단비' 2000만원, '뇌혈관질환진단비' 2000만원, '혈전용해치료비' 200만원... 이런 항목들이 쫙 있더라고요"
- 금액과 함께 담보명을 따옴표로 감싸서 그대로 인용
- 월보험료/납입기간/총납입 등 설계서 숫자도 정확히 인용
- 🚨이익 금액이 있으면 반드시 포함: "${bestProfitText || '(이익 계산 데이터 없음)'}"
- 담보가 진단비/수술비 위주면 "건강보험"으로, 연금/적립 위주면 "연금보험"으로 자연스럽게 언급

【고민/걱정 (4~5줄)】
- 담보 항목이 많은데 다 필요한지 모르겠다는 고민
- ${selectedQuestion ? '특히 "' + selectedQuestion + '" ← 이 질문이 핵심 고민!' : '설계사는 좋다 하는데 본인은 판단 못하겠다는 불안'}
- 초보자답게 솔직한 고민 토로

【질문 3개 (체크박스)】
- "✔️"로 시작
- 🚨🚨 각 질문에 실제 담보명/금액을 반드시 1개 이상 인용!
  - 예: "✔️ '혈전용해치료비' 200만원이면 충분한 건가요?"
  - 예: "✔️ '뇌혈관질환진단비'랑 '뇌혈관질환수술비'가 따로 있는데 둘 다 필요한가요?"
- C-RANK 강화: 각 질문이 네이버 검색 키워드로 기능할 수 있게
- 질문 3개 내용이 매번 달라야 함 (시드 ${seed} 반영)

【마무리 + 참고근거 박스】
- "─────────────"
- "📊 참고 근거 (상품설명서 발췌 기준)"
- 담보 목록/금액/납입기간/보험기간 요약
- 출처: 가입설계서

==================================================
🚨🚨🚨 절대 규칙 (위반 시 전체 무효) 🚨🚨🚨
==================================================
1. 🔴 설계서 숫자만 사용 (1원도 만들지 마세요. 금액 정확히!)
2. 🚨 이미지에 없는 보험회사명/상품명을 절대 추측·생성하지 마세요! 모르면 "이 보험", "건강보험" 등으로!
3. 🔴 설계서의 실제 담보명과 금액을 정확히 인용! (담보명은 따옴표로 감싸기)
4. "~됩니다/~입니다" 금지 → "~인데요/~거든요/~더라고요" 
5. 답을 내리지 마세요 (물어보는 글!)
6. ${isHealthType ? '🚨🚨🚨 건강보험/보장성보험이므로 "은퇴", "연금", "적금", "적금 대신", "환급률", "노후 수입", "연금 준비" 등의 표현 절대 금지! 이 보험은 질병/사고 보장 목적입니다!' : 'SEO 키워드: "' + (analysisData.product_type || '연금보험') + '" 2~3회, "보험상담" 1회'}
7. ⚠️ 같은 입력이어도 시드(${seed})가 다르면 완전히 다른 글이 나와야 합니다
8. 피보험자가 ${ageNum <= 18 ? '미성년자(' + insuredAge + '세)이므로 "부모가 자녀를 위해 가입"하는 맥락으로 작성!' : '성인이므로 본인 가입 맥락으로 작성!'}
9. 이익 금액이 있으면 포함: "${bestProfitText || ''}"

JSON:
{
  "title": "질문형 제목 30~50자",
  "content": "본문 800~1200자 (이익계산+참고근거 포함)",
  "questions_in_content": ["✔️ 질문1", "✔️ 질문2", "✔️ 질문3"],
  "reference_data": {
    "납입기간": "${paymentPeriod}${paymentPeriod && !paymentPeriod.includes('년') ? '년' : ''}",
    "월보험료": "${monthlyPremium}",
    "총납입": "${totalPremium}",
    "해약환급금": "${surrenderInfo || '확인 필요'}",
    "출처": "가입설계서"
      ...
      const qaResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용하는 서비스입니다. 시드=${seed}로 매번 완전히 다른 글을 만드세요. 🚨절대규칙: 1)상품명 "${productName}"을 한 글자도 바꾸지 마세요! 2)회사명 "${company}"를 정확히 사용! 3)숫자를 1원이라도 바꾸면 무효! 4)제목은 반드시 질문형("~까요?") 5)본문 800자 이상 6)🔴이익금액 "${bestProfitText}" 반드시 본문에 포함! 7)SEO/AEO/C-RANK/GEO 최적화. ${ageNum <= 18 ? '피보험자가 ' + insuredAge + '세 미성년자이므로 부모가 자녀를 위해 가입하는 맥락!' : ''} ${isHealthType ? '🚨🚨🚨 이 보험은 건강보험/보장성보험입니다! "은퇴", "연금", "적금", "적금 대신", "환급률", "노후 수입", "연금 준비" 표현 절대 금지! 질병/사고 보장 목적의 글로 작성!' : ''}` },
      ...
      if (qaResponse.ok) {
        ...
        try {
          ...
          console.error('[XIVIX] Q&A JSON 파싱 실패')
        }
      }
      ...
      // V2026.37.110 - 전문가 닉네임 풀: 건강보험/연금보험 분리
      const healthExpertPool = [
        { nick: '보험멘토30년', career: '30년 경력 MDRT', intro: '안녕하세요~ 보험멘토30년입니다 😊' },
        { nick: '보험길잡이FP', career: 'MDRT 15회 달성 재무설계사', intro: '안녕하세요, 보험길잡이FP입니다 ✋' },
        { nick: '보험박사23', career: '23년차 보험계리사', intro: '안녕하세요~ 보험박사23이에요 😊' },
        { nick: '설계사언니', career: '17년차 여성 보험전문가', intro: '안녕하세요! 설계사언니예요 🙋‍♀️' },
        { nick: '보험아저씨', career: '25년 현장 경험 보험인', intro: '반갑습니다, 보험아저씨입니다 😊' },
        { nick: '보장분석가', career: 'CFP·AFPK 보장분석 전문가', intro: '안녕하세요~ 보장분석가입니다!' },
        { nick: '보험리모델러', career: '보험리모델링 전문 20년', intro: '안녕하세요, 보험리모델러입니다 😊' },
        { nick: '보험멘토쌤', career: '전 보험사 교육팀장 28년', intro: '안녕하세요~ 보험멘토쌤이에요 ✨' },
      ]
      const pensionExpertPool = [
        { nick: '보험멘토30년', career: '30년 경력 MDRT', intro: '안녕하세요~ 보험멘토30년입니다 😊' },
        { nick: '보험길잡이FP', career: 'MDRT 15회 달성 재무설계사', intro: '안녕하세요, 보험길잡이FP입니다 ✋' },
        { nick: '보험박사23', career: '23년차 보험계리사', intro: '안녕하세요~ 보험박사23이에요 😊' },
        { nick: '설계사언니', career: '17년차 여성 보험전문가', intro: '안녕하세요! 설계사언니예요 🙋‍♀️' },
        { nick: '보험아저씨', career: '25년 현장 경험 보험인', intro: '반갑습니다, 보험아저씨입니다 😊' },
        { nick: '연금전문가K', career: 'CFP·AFPK 연금 전문가', intro: '안녕하세요~ 연금전문가K입니다!' },
        { nick: '보험리모델러', career: '보험리모델링 전문 20년', intro: '안녕하세요, 보험리모델러입니다 😊' },
        { nick: '보험멘토쌤', career: '전 보험사 교육팀장 28년', intro: '안녕하세요~ 보험멘토쌤이에요 ✨' },
      ]
      const expertPool = isHealthType ? healthExpertPool : pensionExpertPool
      const expert = expertPool[(seed + 3) % expertPool.length]
      
      const answerPrompt = `[🔒 중복 방지: 시드=${seed}, 매번 다른 표현·구조·어휘를 사용하세요]

[당신의 역할]
닉네임: "${expert.nick}" (${expert.career})
네이버 보험 카페 인기 답변러. 초보자 질문에 친절하고 구체적으로 답변.
...
${selectedQuestion ? `(초보자가 올린 질문입니다!)
🚨🚨🚨 초보자 눈높이 답변 규칙:
- 초보자가 "이게 뭐예요?"라고 물었으면 → 그 용어를 일상 비유로 쉽게 설명!
  예: "'진단비'라는 건요, 병원에서 '이 병이에요'라고 확정 진단을 받으면 그때 딱 한 번 돈이 나오는 거예요"
  예: "'특약'이라는 건 기본 보험에 추가로 붙이는 옵션 같은 거예요. 뷔페에서 기본 메뉴 말고 추가 메뉴 고르는 것처럼요"
- 전문 용어를 쓸 때는 반드시 괄호 안에 쉬운 설명을 넣으세요
  예: "납입면제(보험료 안 내도 되는 혜택)"
  예: "갱신형(일정 기간마다 보험료가 올라가는 방식)"
- 초보자가 이해할 수 있도록 비유와 예시를 적극 활용!` : ''}

` : ''}[설계서 데이터]
• 피보험자: ${insuredAge}세 ${analysisData.insured_gender || ''}
• 납입: ${paymentPeriod}년, 월 ${monthlyPremium}, 총 ${totalPremium}
• 해약환급금: ${surrenderInfo || '확인 필요'}
• 공시이율: ${analysisData.interest_rate || '확인 필요'}
• 이익 금액: ${bestProfitText || '확인 필요'}

🚨🚨🚨 [설계서 담보/특약 목록 - 답변에 반드시 구체적으로 인용!] 🚨🚨🚨
${benefits || '담보 정보 확인 필요'}
→ 위 담보명들을 따옴표로 감싸서 답변에 5개 이상 구체적으로 언급하세요!
→ 각 담보의 보장금액도 함께 언급하세요!
→ 🚨 담보 목록의 모든 담보를 가능한 한 빠짐없이 언급하세요! 최소 5개, 가능하면 전체!

🚨🚨🚨 [보험종류 자동 판별 - 필수!] 🚨🚨🚨
- 담보에 "진단비", "수술비", "치료비", "입원" 등이 있으면 → 이 보험은 "건강보험/보장성보험"
- 담보에 "연금", "적립", "환급금" 위주면 → 이 보험은 "연금보험/저축성보험"
- 🚨 건강보험인데 "연금보험"이라고 답변하면 전체 무효!
- 🚨 이미지에 없는 보험회사명/상품명을 절대 추측·생성하지 마세요! 모르면 "이 보험", "해당 상품"으로 표현!

==================================================
📝 전문가 답변 (800~1200자, 순수 텍스트만)
==================================================

"${expert.intro}"로 시작하세요.

내용 구성 (순서와 비중은 자연스럽게 변형 가능):
■ 질문자 상황 공감 + 격려
■ 🚨🚨🚨 설계서의 실제 담보명을 구체적으로 인용하며 분석! (최소 5개 이상!)
  - 예: "'고액치료비암진단비' 2,000만원은 암 진단 시 한 번 지급되는 건데요, 이 정도면 기본적인 치료비는 커버가 돼요"
  - 예: "'혈전용해치료비' 200만원은 혈전 관련 치료 시 1회 지급되는 특약인데요"
  - 예: "'뇌혈관질환진단비' 2,000만원, '허혈성심장질환진단비' 2,000만원처럼 뇌·심장 진단비가 탄탄하네요"
  - 반드시 담보명 5개 이상을 따옴표로 감싸서 구체적으로 설명!
  - 🚨 담보 목록에 있는 모든 담보를 가능한 한 빠짐없이 언급하세요!
■ 이익 금액이 있으면 포함: "${bestProfitText || ''}"
■ 설계서 숫자 기반 객관적 평가
■ 주의할 점 2~3가지 (중도해지 위험, 갱신 여부, 보장 범위 등)
■ 다른 보험사 비교 추천 ("같은 보장이라도 보험사마다 보험료가 다르거든요")
■ "2~3개 보험사 설계서를 비교해보시면 좋겠어요"
■ 따뜻한 마무리
...
- "~됩니다" 금지 → "~거든요", "~해볼 수 있어요", "~검토해보시면 좋겠어요"
- 🚨 이미지에 없는 보험회사명/상품명 절대 사용 금지! 모르면 "이 보험"으로!
- 🚨 담보명은 설계서에 있는 그대로 따옴표로 감싸서 인용!
- 설계서 숫자만! 없는 숫자 만들기 금지
- 시드(${seed})를 반영해 같은 상품이어도 매번 다른 문장·어휘·비유를 사용
- 800~1200자 (담보가 많으면 더 길어도 됨)`
      ...
      const answerResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용합니다. 시드=${seed}로 매번 완전히 다른 표현·문체·비유 사용. 당신은 "${expert.nick}" (${expert.career})으로, 네이버 보험 카페 인기 답변러. 🚨절대규칙: 1)설계서 담보명을 따옴표로 감싸서 5개 이상 구체적으로 인용(가능하면 전체)! 2)설계서 숫자만 사용, 환각 금지! 3)이미지에 없는 보험회사명/상품명을 절대 추측·생성하지 마세요! 모르면 "이 보험"으로! 4)담보에 진단비/수술비/치료비가 있으면 건강보험, 연금/적립이 있으면 연금보험! 건강보험인데 연금보험이라 쓰면 무효! 5)이익금액이 있으면 포함: "${bestProfitText || ''}" 순수 텍스트 800~1200자. ${isHealthType ? '🚨🚨🚨 이 보험은 건강보험/보장성보험! "은퇴", "연금", "적금 대신", "환급률", "노후 수입" 표현 절대 금지! 담보(진단비/수술비/치료비) 중심으로 답변!' : ''}` },
      ...
      if (answerResponse.ok) {
        ...
        expertAnswer = json.choices?.[0]?.message?.content || ''
      }
      ...
      // 닉네임 랜덤 풀 (매번 다른 조합)
      const nickPools = {
        empathy: ['뽀뽀맘','두아이아빠','맘스카페','워킹맘이안','살림왕지영','아이셋맘','육아중독','행복한아빠','소소한일상','맘카페단골'],
        info: ['똑소리엄마','보험공부중','현명한소비','재테크맘','알뜰살뜰','비교왕언니','머니톡톡','절약하는남편','슬기로운보험생활','돈공부하는엄마'],
        question: ['하늘아래','오래사는비결','보험고민중','꼼꼼이','궁금이','나도알고싶다','초보탐험가','비교중입니다','신중한선택','알아보는중']
      }
      const nick1 = nickPools.empathy[(seed + 1) % nickPools.empathy.length]
      const nick2 = nickPools.info[(seed + 5) % nickPools.info.length]
      const nick3 = nickPools.question[(seed + 7) % nickPools.question.length]
      
      const commentPrompt = `[🔒 중복 방지: 시드=${seed}, 닉네임·내용·말투가 매번 달라야 합니다]

[원글 제목] ${qaData.title || '보험 상품 문의'}
[원글 핵심] ${paymentPeriod}년납 월 ${monthlyPremium}, 담보: ${benefits || '보험 담보'}
[보험종류] ${isHealthType ? '건강보험/보장성보험 (질병·사고 보장)' : analysisData.product_type || '보험상품'}
${isHealthType ? '🚨 이 보험은 건강보험입니다! 댓글에 "은퇴", "연금", "적금", "환급률" 등을 절대 언급하지 마세요! 담보(진단비/수술비/치료비) 관련 경험만!' : ''}

==================================================
💬 댓글 3개 (실제 네이버 카페 회원처럼!)
==================================================

【댓글1: 공감형】 닉네임: "${nick1}"
- "저도 비슷한 상황이었는데~" + 본인 경험 + 느낌
- 100~150자, 친근한 존댓말, 이모지 0~1개

【댓글2: 정보형】 닉네임: "${nick2}"  
- 비교 경험이나 구체적 팁 (예: "저는 3곳 비교해봤는데~")
- 120~180자, 보험 관련 키워드 포함
- 🚨 이미지에 없는 보험회사명은 절대 언급하지 마세요!

【댓글3: 질문형】 닉네임: "${nick3}"
- "혹시 ~도 확인해보셨나요?" 형태로 새로운 관점 제시
- 80~120자, 원글에 없는 포인트 (${isHealthType ? '갱신 여부, 납입면제, 보장 범위, 중복보장 등' : '비과세, 중도인출, 타사비교 등'})

🔒 규칙: 줄바꿈 없이 1~3문장, AI 느낌 금지, 시드(${seed})로 내용 변형
🚨 이미지에 없는 보험회사명/상품명 절대 사용 금지! 모르면 "이 보험", "해당 상품"으로!

JSON: {"comments": [{"nickname": "${nick1}", "text": "..."}, {"nickname": "${nick2}", "text": "..."}, {"nickname": "${nick3}", "text": "..."}]}`
      ...
      const commentResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용합니다. 시드=${seed}로 매번 다른 내용·어휘·경험담을 만드세요. 네이버 카페 실제 회원 3명을 연기합니다. AI 느낌 절대 금지. 각 댓글은 성격·경험·말투가 확실히 달라야 합니다. 🚨이미지에 없는 보험회사명/상품명 절대 사용 금지! ${isHealthType ? '🚨건강보험/보장성보험이므로 "은퇴", "연금", "적금" 언급 금지! 질병/사고 보장 맥락으로!' : ''}` },
            { role: 'user', content: commentPrompt }
          ],
          max_tokens: 1200,
          temperature: 0.85
      ...
      const hashtagPrompt = `[시드=${seed} - 매번 다른 조합!]

[상품 정보] ${analysisData.product_type || '보험상품'}, ${insuredAge}세, ${paymentPeriod}년납
[담보 키워드] ${benefits || '보험'}

해시태그 10개 생성 (네이버 SEO/C-RANK/AEO/GEO 최적화):

■ 구성:
- 보험 유형 (2~3개): 담보에 진단비/수술비/치료비가 있으면 #건강보험 #보장성보험 관련, 연금/적립이 있으면 #연금보험 관련
- 담보 키워드 (2~3개): 실제 담보명에서 추출 (예: #암진단비 #뇌혈관보장 #심장질환보험)
- 행동 유도 (2~3개): #보험상담 #보험비교 #설계서분석 등
- 정보 키워드 (1~2개): #보험설계서 #보험리모델링 등

■ 규칙:
- #으로 시작, 띄어쓰기 없이
- 정확히 10개, 중복 절대 금지
- 🚨🚨 이미지/설계서에 보험회사명이 없으면 회사명 해시태그 절대 금지! 추측·생성 금지!
- 🚨 이미지에 없는 보험회사명(예: #삼성생명 #한화생명)을 임의로 만들지 마세요!
- 회사명이 확실히 있을 때만 해시태그에 포함
- 네이버에서 일반인이 실제 검색할 키워드만
- 시드(${seed})로 매번 다른 조합

JSON: {"hashtags": ["#태그1","#태그2","#태그3","#태그4","#태그5","#태그6","#태그7","#태그8","#태그9","#태그10"]}`
      ...
      const hashtagResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `네이버 SEO/C-RANK/AEO/GEO 전문가. 시드=${seed}로 매번 다른 키워드 조합 생성. 중복 태그 금지. 실제 검색량 높은 키워드만. 🚨절대규칙: 이미지/설계서에 보험회사명이 명확히 없으면 회사명 해시태그를 절대 만들지 마세요! 추측·날조 금지!` },
      ...
      const finalResult = {
        ...
        qa: {
          ...
          questions_in_content: qaData.questions_in_content || [],
          reference_data: qaData.reference_data || {}
        },
      ...
      console.log('[XIVIX] 보험 Q&A 생성 완료:', finalResult.qa.title)
      
    } catch (error: any) {
      console.error('[XIVIX] 보험 Q&A 생성 오류:', error)
...
/* V2026.37.101 - AI 모델 선택 토글 (모바일 최적화) */
.ai-model-selector{
  display:flex;
  justify-content:center;
  margin-bottom:20px;
...
/* ============================================
   V2026.37.101 - 전체 화면 로딩 오버레이 (CEO 긴급 지시)
...
/* V2026.37.101 - 로딩 중 실시간 정보 표시 (CEO 지시: 뉴스 자막 흐르게) */
...
/* V2026.37.101 - 이미지1: 눈에 잘 보이게 크게 표시 */
...
<!-- V2026.37.101 - 카카오 SDK 제거 (CEO 지시: 카카오 로그인 안되니 지워) -->

<!-- ✅ V2026.37.101 - PWA 지원 (바탕화면 바로가기) -->
...
<div class="fullscreen-loading-overlay" id="fullscreenLoading">
  <div class="fullscreen-loading-content">
    ...
    <!-- V2026.37.101 - 실시간 정보 표시 (CEO 지시: 뉴스 자막 흐르게) -->
...
<div class="login-modal" id="loginModal">
  <div class="login-modal-content" style="position:relative">
    ...
    <!-- V2026.37.101 - 카카오 로그인 제거 (CEO 지시: 카카오 로그인 안되니 지워) -->
...
<nav class="nav">
  <!-- V2026.37.101 - Admin 버튼은 관리자(010-4845-3065)만 볼 수 있음 -->
...
<div class="wrapper">
  ...
  <div class="main">
    ...
    <!-- V2026.37.101 - AI 모델 선택 (Gemini / GPT-4o 둘 다 보이게) -->
    ...
    <div class="result-section" id="resultSection">
      ...
      <!-- V2026.37.101 - CEO 지시: 새로고침/바로가기 버튼 삭제 -->
      
      <!-- 🖼️ V2026.37.101 - 마케팅 이미지 (업로드된 설계서 사용) -->
...
// ✅ V2026.37.101 - 전역 로딩 오버레이 요소 (중복 선언 에러 방지)
...
// ✅ V2026.37.101 - 실시간 시세/뉴스 API 연동 (하드코딩 제거)
...
// ✅ V2026.37.101 - 실시간 시세 API 호출 (백엔드 통합 - CORS 우회)
...
function startRealTimeData() {
  ...
  // 🔥 V2026.37.101 - 시세 30초마다 갱신 (실시간 느낌)
...
// V2026.37.101 - 결과 데이터 복원 (24시간 유지)
...
fileInput.addEventListener('change', async (e) => {
  ...
  // 🔥 V2026.37.104 - 이미지 업로드 시 텍스트 보존! (텍스트=질문, 이미지=OCR 보조)
  // 텍스트가 있으면 사용자 질문으로 우선 사용, OCR은 보조 데이터로 활용
  if (files.length > 0) {
    const searchInput = document.getElementById('search');
    if (searchInput) {
      const currentText = searchInput.value.trim();
      if (currentText) {
        // 텍스트가 있으면 보존하고 안내만 변경
        searchInput.placeholder = '📷 이미지 + 텍스트 함께 분석됩니다';
        console.log('[XIVIX] 📷 텍스트 보존됨 (질문 우선):', currentText.substring(0, 30));
      } else {
        // 텍스트 없으면 OCR 전용 모드
        searchInput.placeholder = '📷 이미지 업로드됨 - OCR 분석으로 자동 생성 (질문 입력도 가능!)';
        console.log('[XIVIX] 📷 텍스트 없음 → OCR 전용 모드');
      }
    }
  }
...
function removeFile(id) {
  ...
  // 🔥 V2026.37.101 - 이미지 모두 삭제 시 placeholder 복원
...
// ============================================
// ✅ V2026.37.101 - 전체 새로고침 기능 (CEO 요청)
...
// 트렌드 로드 (Linear 스타일 미니멀 UI) - V2026.37.101 null 체크 추가
...
// 새로고침 버튼 클릭 (에러 가드 포함) - V2026.37.101 디버깅 강화
...
// V2026.37.101 - AI 모델 선택 (gemini | gpt)
let selectedAIModel = 'gpt'; // V2026.37.101 - GPT-4o 기본값
...
// 페이지 로드 시 저장된 모델 복원 (V2026.37.101 - GPT-4o 기본값)
...
const repairModalCloseButtons = () => {
    ...
    // 로그인 모달 X 버튼 (V2026.37.101 수정: 올바른 클래스명 .login-close)
...
// ✅ V2026.37.101 - 인덱스로 해시태그 복사 (문자열 이스케이프 문제 해결)
...
// ✅ V2026.37.101 - TXT 파일 다운로드 (CEO 지시)
...
// V2026.37.101 - 관리자 전용: Admin 버튼 표시
...
function checkApiLimit() {
  // ✅ V2026.37.101 - CEO 긴급 지시: 모든 사용자 무제한!
...
// ============================================
// ✅ V2026.37.99 - 보험 설계서 Q&A 스트리밍 생성 (GPT-4o + 다중 이미지)
// CEO 지시: 텍스트 + 이미지 1~10장 함께 분석
// 출력: 제목 + 질문 1개 + 답변 1개 + 댓글 3개 + 해시태그
// ============================================
...
// 출력: 제목 + 질문 1개 + 답변 1개 + 댓글 3개 + 해시태그
function renderInsuranceQAResults(data) {
  const resultSection = document.getElementById('resultSection');
  ...
  if (data.analysis) {
    ...
    // 상품 정보
    html += '<div style="padding:12px;background:rgba(255,255,255,0.05);border-radius:8px">';
    html += '<div style="font-size:11px;color:var(--primary);margin-bottom:4px">📋 상품 정보</div>';
    html += '<div style="font-size:14px;font-weight:600">' + (a.company || '-') + ' ' + (a.product_name || '-') + '</div>';
    html += '<div style="font-size:12px;color:var(--text-muted)">' + (a.product_type || '-') + '</div>';
    html += '</div>';
    
    // 계약 조건
    html += '<div style="padding:12px;background:rgba(255,255,255,0.05);border-radius:8px">';
    html += '<div style="font-size:11px;color:var(--green);margin-bottom:4px">👤 계약 조건</div>';
    html += '<div style="font-size:14px">' + (a.insured_age || '-') + '세 ' + (a.insured_gender || '') + '</div>';
    html += '<div style="font-size:12px;color:var(--text-muted)">납입기간: ' + (a.payment_period || '-') + '년</div>';
    html += '</div>';
    
    // 보험료 정보
    html += '<div style="padding:12px;background:rgba(255,255,255,0.05);border-radius:8px">';
    html += '<div style="font-size:11px;color:var(--orange);margin-bottom:4px">💰 보험료</div>';
    html += '<div style="font-size:14px;font-weight:600">월 ' + (a.monthly_premium || '-') + '</div>';
    html += '<div style="font-size:12px;color:var(--text-muted)">총 납입: ' + (a.total_premium || '-') + '</div>';
    html += '</div>';
    
    // 해약환급금 정보
    if (a.surrender_values && a.surrender_values.length > 0) {
      html += '<div style="padding:12px;background:rgba(255,255,255,0.05);border-radius:8px">';
      html += '<div style="font-size:11px;color:var(--accent);margin-bottom:4px">📈 해약환급금</div>';
      a.surrender_values.forEach(sv => {
        html += '<div style="font-size:13px">' + sv.year + '년: ' + sv.amount + ' (' + sv.rate + ')</div>';
      });
      html += '</div>';
    }
  ...
  if (data.qa) {
    ...
    html += '<div class="section-header"><i class="fas fa-question-circle"></i> ❓ 카페 질문글</div>';
    html += '<div class="section-content">';
    ...
    // 본문
    html += '<div style="font-size:14px;line-height:1.8;color:var(--text);white-space:pre-wrap">';
    html += (data.qa.content || '').replace(/\\n/g, '<br>');
    html += '</div>';
    
    // 본문 내 질문들
    if (data.qa.questions_in_content && data.qa.questions_in_content.length > 0) {
      html += '<div style="margin-top:16px;padding:12px;background:rgba(79,140,255,0.1);border-radius:8px">';
      html += '<div style="font-size:12px;color:var(--primary);margin-bottom:8px">✔️ 궁금한 점</div>';
      data.qa.questions_in_content.forEach(q => {
        html += '<div style="font-size:13px;margin-bottom:4px">• ' + q + '</div>';
      });
      html += '</div>';
    }
    
    // 참조 데이터
    if (data.qa.reference_data) {
      const ref = data.qa.reference_data;
      html += '<div style="margin-top:12px;padding:10px;background:rgba(255,255,255,0.03);border-radius:6px;font-size:12px;color:var(--text-muted)">';
      html += '📊 참고: ';
    ...
    html += '<button class="copy-btn" onclick="copyInsuranceQuestion()"><i class="fas fa-copy"></i> 질문글 복사</button>';
    html += '</div>';
  ...
  if (data.expert_answer) {
    html += '<div class="sequential-section show" style="margin-top:20px">';
    html += '<div class="section-header"><i class="fas fa-user-tie"></i> 💬 전문가 답변</div>';
    html += '<div class="section-content">';
    html += '<div class="item-card" style="border-left:4px solid var(--green);background:rgba(16,185,129,0.05)">';
    
    html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">';
    html += '<div style="width:40px;height:40px;background:var(--green);border-radius:50%;display:flex;align-items:center;justify-content:center"><i class="fas fa-user-md" style="color:white"></i></div>';
    html += '<div><div style="font-weight:600">MDRT 보험전문가</div><div style="font-size:11px;color:var(--text-muted)">30년 경력 · 인증 전문가</div></div>';
    html += '</div>';
    
    html += '<div style="font-size:14px;line-height:1.8;color:var(--text);white-space:pre-wrap">';
    html += (data.expert_answer || '').replace(/\\n/g, '<br>');
    html += '</div>';
    
    html += '<div style="margin-top:16px;display:flex;gap:8px">';
    html += '<button class="copy-btn" onclick="copyInsuranceAnswer()"><i class="fas fa-copy"></i> 답변 복사</button>';
    html += '</div>';
  ...
  if (data.comments && data.comments.length > 0) {
    html += '<div class="sequential-section show" style="margin-top:20px">';
    html += '<div class="section-header"><i class="fas fa-comments"></i> 💭 댓글 (' + data.comments.length + '개)</div>';
    html += '<div class="section-content">';
    
    data.comments.forEach((c, idx) => {
      html += '<div class="item-card" style="margin-bottom:12px;padding:12px">';
      html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">';
      html += '<div style="width:32px;height:32px;background:var(--card-bg);border:1px solid var(--border);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px">👤</div>';
      html += '<div style="font-weight:500;font-size:13px">' + (c.nickname || '회원' + (idx+1)) + '</div>';
      html += '</div>';
      html += '<div style="font-size:14px;color:var(--text);padding-left:40px">' + (c.text || '') + '</div>';
      html += '</div>';
    });
    
    html += '<div style="display:flex;gap:8px">';
    html += '<button class="copy-btn" onclick="copyInsuranceComments()"><i class="fas fa-copy"></i> 댓글 전체 복사</button>';
    html += '</div>';
  ...
  if (data.hashtags && data.hashtags.length > 0) {
    html += '<div class="sequential-section show" style="margin-top:20px">';
    html += '<div class="section-header"><i class="fas fa-hashtag"></i> 🏷️ 해시태그</div>';
    html += '<div class="section-content">';
    html += '<div class="item-card">';
    html += '<div style="display:flex;flex-wrap:wrap;gap:8px">';
    data.hashtags.forEach(tag => {
      html += '<span class="keyword-tag" onclick="copyText(\\'' + tag.replace(/'/g, "\\\\'") + '\\')" style="cursor:pointer;font-size:13px;background:var(--primary-soft);color:var(--primary)">' + tag + '</span>';
    });
    html += '</div>';
    html += '<div style="margin-top:12px">';
    html += '<button class="copy-btn" onclick="copyInsuranceHashtags()"><i class="fas fa-copy"></i> 해시태그 전체 복사</button>';
    html += '</div>';
    html += '</div></div></div>';
  }
  ...
  // 결과 데이터 저장
  window.insuranceQAData = data;
}

// ✅ V2026.37.99 - 보험 Q&A 복사 함수들
function copyInsuranceQuestion() {
  ...
  navigator.clipboard.writeText(text).then(() => {
    showCopyToast('질문글 복사 완료!');
  });
}

function copyInsuranceAnswer() {
  const data = window.insuranceQAData;
  if (!data?.expert_answer) return;
  
  navigator.clipboard.writeText(data.expert_answer).then(() => {
    showCopyToast('전문가 답변 복사 완료!');
  });
}

function copyInsuranceComments() {
  const data = window.insuranceQAData;
  if (!data?.comments?.length) return;
  
  let text = data.comments.map(c => (c.nickname || '회원') + ': ' + (c.text || '')).join('\\n');
  
  navigator.clipboard.writeText(text).then(() => {
    showCopyToast('댓글 ' + data.comments.length + '개 복사 완료!');
  });
}

function copyInsuranceHashtags() {
  const data = window.insuranceQAData;
...
async function goGenerateStream() {
  ...
  // 🔥 V2026.37.101 - 이미지 업로드 시 텍스트 입력 완전 무시!
  ...
  // ============================================
  // ✅ V2026.37.101 - 전체 화면 로딩 오버레이 표시 (CEO 긴급 지시)
  // 문제: 생성 중 복사 버튼 누르고 "고장났다" 연락
  // 해결: 생성 완료까지 전체 화면 차단
  // ============================================
  ...
  if (fullscreenOverlay) {
    ...
    // V2026.37.101 - 실시간 데이터 시작 (CEO 지시: 기다리다 잠들겠다!)
    startRealTimeData();
  }
  ...
  const requestData = { 
    concern: q,
    aiModel: selectedAIModel  // V2026.37.101 - AI 모델 선택 전달
  };
  if (uploadedFiles.length > 0) {
    requestData.image = uploadedFiles[0].base64;
    requestData.mimeType = uploadedFiles[0].type;
        ...
        try {
          ...
          switch (event.type) {
            case 'step':
              ...
              // ✅ V2026.37.101 - 전체 화면 오버레이 진행 상황 업데이트
            ...
            case 'complete':
              ...
              // ✅ V2026.37.101 - 전체 화면 오버레이 완료 표시 후 숨김
              if (fullscreenOverlay) {
              ...
              setTimeout(() => {
                ...
                // V2026.37.101 - CEO 지시: 3단계 자동화 (분석→정리→이미지 생성)
                // 보험 정보가 감지된 경우 자동으로 마케팅 이미지 생성 (company는 topic에서 추출 가능)
                if (resultData && resultData.insurance) {
                  console.log('[XIVIX] V2026.37.101 자동 이미지 생성 시작:', resultData.company || '(topic에서 추출 예정)', resultData.insurance);
                  // 토스트 알림
      ...
      try {
        ...
        if (event.type === 'complete') {
          ...
          setTimeout(() => {
            ...
            // V2026.37.101 - CEO 지시: 3단계 자동화 (분석→정리→이미지 생성)
            if (resultData && resultData.insurance) {
              console.log('[XIVIX] V2026.37.101 자동 이미지 생성 (버퍼):', resultData.company || '(topic에서 추출 예정)', resultData.insurance);
              setTimeout(() => {
                generateMarketingImage();
              }, 1500);
            }
  ...
  // ✅ V2026.37.101 - 에러/완료 시 전체 화면 오버레이 숨김
  fullscreenOverlay = document.getElementById('fullscreenLoading');
  if (fullscreenOverlay) {
    fullscreenOverlay.classList.remove('show');
  }
...
// ============================================
// ✅ V2026.37.101 - CEO 지시: 바탕화면 바로가기 추가
// 휴대폰/PC 모두 지원
// ============================================
...
// V2026.37.101 - 트렌드 새로고침 버튼 이벤트 강제 바인딩 (모바일 대응)
...
function downloadUploadedImage() {
  ...
  try {
    ...
    console.log('[XIVIX] V2026.37.101 이미지 다운로드 완료');
  } catch (e) {
...
async function generateMarketingImage() {
  // ✅ V2026.37.101 - CEO 지시: DALL-E/XIIM API 제거, 업로드된 이미지만 표시
  ...
  if (uploadedFiles && uploadedFiles.length > 0 && uploadedFiles[0].base64) {
    ...
    console.log('[XIVIX] V2026.37.101 업로드된 설계서 이미지 표시 완료');
  } else {
    ...
    console.log('[XIVIX] V2026.37.101 업로드된 이미지 없음 - 안내 메시지 표시');
  }
  
  // ✅ V2026.37.101 - 기존 DALL-E/XIIM API 호출 코드 비활성화 (조기 리턴)
  return;
  
  // ============ 아래 코드는 실행되지 않음 (V2026.37.101) ============
  ...
  // ============================================
  // ✅ V2026.37.101 - CEO 긴급 지시: 하이브리드 UI 시스템
  ...
  // ✅ V2026.37.101 - 전역 제외 URL 동기화
  // globalExcludedUrls를 사용하여 세션 동안 중복 방지
  
  // V2026.37.101 - XIIM API V2.2 규격에 맞춘 keyword 생성
  // 공식: {보험사 한글명} {상품유형} {설계안/설계서}
  
  // V2026.37.101 - 보험사명 추출 우선순위:
  ...
  if (!company) {
    ...
    for (const c of companyList) {
      if (topicLower.includes(c.toLowerCase()) || topicLower.includes(c.replace('생명', '').replace('손해보험', '').replace('화재', ''))) {
        company = c;
        console.log('[XIVIX] V2026.37.101 topic에서 보험사 추출:', company);
        break;
      }
    }
  ...
  try {
    ...
    if (!hasDirectUrl) {
      console.log('[XIVIX] V2026.37.101 하이브리드 UI - 후보 3개 병렬 검색 시작');
      ...
      if (validCandidates.length > 0) {
        console.log('[XIVIX] V2026.37.101 하이브리드 UI 성공 - 유효 후보:', validCandidates.length);
    ...
    // ============================================
    // ✅ V2026.37.101 - XIIM API V2.4 재시도 로직 (미들웨어 수정 완료 대응)
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        response = await fetch('/api/xiim/process', {
          ...
          body: JSON.stringify({
            ...
            request_info: {
              ...
              keyword: currentKeyword,             // ✅ V2026.37.101: 변형 키워드 사용
              target_company: targetCompany,       // ❗ 필수: keyword와 일치해야 함!
              title: selectedTitleText,            // 선택: 검색 정확도 향상
              exclude_urls: excludedUrls.length > 0 ? excludedUrls : undefined, // ✅ V2026.37.101: 제외 URL
        ...
        // ✅ V2026.37.101 - Rate Limit 처리 (HTTP 429)
    ...
    // ============================================
    // ✅ V2026.37.101 - XIIM V2.7 대응: success 필드 확인 강화
    ...
    // ============================================
    // ✅ V2026.37.101 - CEO 긴급 지시: 잘못된 이미지 패턴 감지 및 자동 재검색
    // "삼성금융 Open Collaboration", 홍보 포스터, 광고 이미지 등 필터링
    // 이 패턴이 감지되면 자동으로 재생성 시도
    // ============================================
    // ============================================
    // ✅ V2026.37.101 - CEO 긴급 지시: 홍보물/광고 이미지 필터 강화
    ...
    if (isInvalidPattern || isPromoImage) {
      ...
      // ✅ V2026.37.101: 자동 재시도 - throw 대신 continue
    ...
    // ============================================
    // ✅ V2026.37.101 - XIIM V2.8 대응: verification 필드 완전 활용
    // ============================================
    ...
    console.log('[XIVIX] V2026.37.101 이미지 검증 결과:', { 
      isDesignDocument, 
      detectedCompany, 
    ...
    // ✅ V2026.37.101: XIIM V2.8 - 설계서가 아닌 경우 자동 재시도 (더 정확한 판별)
    ...
    // ✅ V2026.37.101: 설계서로 판정된 경우 성공 로그
    ...
    if (detectedCompany && detectedCompany !== company && detectedCompany !== 'UNKNOWN') {
      ...
      // ✅ V2026.37.101: 유효한 이미지 찾음 - while 루프 종료
      break;
      
    } // end of while (!validImageFound && autoRetryCount < MAX_AUTO_RETRY)
    
    // ============================================
    // ✅ V2026.37.101: 모든 시도 후 결과 확인
    // ============================================
    if (!validImageFound) {
    ...
    if (error.message.includes('Failed to fetch') || error.name === 'TypeError') {
      ...
      // ✅ V2026.37.101 - 홍보/광고 이미지 감지 시 자동 재시도
...
// ============================================
// ✅ V2026.37.101 - CEO 긴급 지시: 이미지 재생성 기능
...
async function regenerateMarketingImage() {
  ...
  regenerateAttempts++;
  console.log('[XIVIX] V2026.37.101 재생성 요청 #' + regenerateAttempts);
  ...
  try {
    ...
    console.log('[XIVIX] V2026.37.101 재생성:', company, insurance);
...
// ✅ V2026.37.101 - 레거시 코드 제거됨 (이미지 1개만 생성)
...
// ✅ V2026.37.101 - 후보 선택 UI 제거됨 (1개만 자동 표시)
...
// ✅ V2026.37.101 - 후보 보기 기능 제거 (더미 함수)
function showImageCandidates() {
  console.log('[XIVIX] 후보 보기 기능 비활성화됨 (1개만 표시)');
}

// ============================================
// ✅ V2026.37.101 - 이미지 1개만 생성 (DALL-E 3 우선)
// 후보 선택 UI 제거, 1개만 자동 표시
// ============================================
async function fetchImageCandidates(company, insurance, title) {
  // ✅ V2026.37.101 - 이미지 1개만 생성 (DALL-E 3 우선 → 검색 폴백)
  console.log('[XIVIX] V2026.37.101 이미지 생성 시작 (1개만):', { company, insurance });
...
// V2026.37.101 - 이미지2: 관리자 페이지 보안 (010-4845-3065 방익주 대표만 접근 가능)
// 관리자 인증 페이지
...
<script>
// V2026.37.101 - 관리자: 방익주(010-4845-3065), 김미경(010-3159-3697)
