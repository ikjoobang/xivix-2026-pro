  ...
  "advice": "전문가 핵심 조언"
      ...
      if (visionResponse.ok) {
        ...
        try {
          ...
          // V2026.37.57 - 이미지에서 추출한 보험사명 저장 (자동 이미지 생성용)
  ...
  "comments": [
    ...
    return c.json({
      ...
      package: {
        ...
        // V2026.37.57 - CEO 지시: 보험사명 추가 (자동 이미지 생성용)
...
app.post('/api/generate/full-package-stream', async (c) => {
  ...
  return streamText(c, async (stream) => {
    try {
      ...
      // V2026.37.57 - 이미지에서 추출한 보험사/상품명 (자동 이미지 생성용)
...
이미지에 없는 정보는 null로 표시하세요.`
        ...
        if (visionResponse.ok) {
          ...
          try {
            ...
            // V2026.37.57 - 보험사명 저장 (자동 이미지 생성용)
          ...
          switch (event.type) {
            ...
            case 'complete':
              ...
              setTimeout(() => {
                ...
                // V2026.37.57 - CEO 지시: 3단계 자동화 (분석→정리→이미지 생성)
                // 보험 정보가 감지된 경우 자동으로 마케팅 이미지 생성 (company는 topic에서 추출 가능)
                if (resultData && resultData.insurance) {
                  console.log('[XIVIX] V2026.37.57 자동 이미지 생성 시작:', resultData.company || '(topic에서 추출 예정)', resultData.insurance);
                  // 토스트 알림
      ...
      try {
        ...
        if (event.type === 'complete') {
          ...
          setTimeout(() => {
            ...
            // V2026.37.57 - CEO 지시: 3단계 자동화 (분석→정리→이미지 생성)
            if (resultData && resultData.insurance) {
              console.log('[XIVIX] V2026.37.57 자동 이미지 생성 (버퍼):', resultData.company || '(topic에서 추출 예정)', resultData.insurance);
              setTimeout(() => {
                generateMarketingImage();
              }, 1500);
            }
...
async function generateMarketingImage() {
  ...
  // V2026.37.57 - XIIM API V2.2 규격에 맞춘 keyword 생성
  // 공식: {보험사 한글명} {상품유형} {설계안/설계서}
  
  // V2026.37.57 - 보험사명 추출 우선순위:
  ...
  if (!company) {
    ...
    for (const c of companyList) {
      if (topicLower.includes(c.toLowerCase()) || topicLower.includes(c.replace('생명', '').replace('손해보험', '').replace('화재', ''))) {
        company = c;
        console.log('[XIVIX] V2026.37.57 topic에서 보험사 추출:', company);
        break;
      }
    }
  ...
  try {
    ...
    // ============================================
    // ✅ V2026.37.57 - XIIM API V2.4 재시도 로직 (미들웨어 수정 완료 대응)
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // ✅ V2026.37.57 - Rate Limit 처리 (HTTP 429)
    ...
    // ============================================
    // ✅ V2026.37.57 - XIIM V2.7 대응: success 필드 확인 강화
    ...
    // ============================================
    // ✅ V2026.37.57 - CEO 긴급 지시: 잘못된 이미지 패턴 감지 및 자동 재검색
    ...
    if (isInvalidPattern || isPromoImage) {
      console.error('[XIVIX] ❌ 잘못된 이미지 패턴 감지:', { url: imageUrl, source: imageSource });
      console.log('[XIVIX] 🔄 자동 재검색 트리거...');
      
      // 잘못된 이미지 URL을 제외 목록에 추가하고 자동 재시도
      excludedImageUrls.push(imageUrl);
      throw new Error('INVALID_IMAGE_TYPE: 홍보/광고 이미지가 감지되었습니다. 설계서 이미지를 다시 검색합니다.');
    }
    
    // ============================================
    // ✅ V2026.37.57 - 샘플/폴백 이미지 감지 및 경고
    ...
    // ============================================
    // ✅ V2026.37.57 - CEO 긴급 지시: 보험 설계서 검증
    ...
    if (error.message.includes('Failed to fetch') || error.name === 'TypeError') {
      ...
      // ✅ V2026.37.57 - 홍보/광고 이미지 감지 시 자동 재시도
...
// ============================================
// ✅ V2026.37.57 - CEO 긴급 지시: 이미지 재생성 기능
