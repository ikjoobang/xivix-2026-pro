...
JSON 형식으로만 응답:
{
  "company": "보험회사명 (이미지에서 추출)",
  "product_name": "상품명 (이미지에서 추출, 변경 금지)",
  "product_type": "보험종류",
  "insured_age": "피보험자 나이",
  "insured_gender": "성별",
...
1. 상품명: 이미지에 인쇄된 상품명을 "한 글자도 바꾸지 말고" 그대로 출력
   - "KB 트리플 레벨업 연금보험 무배당(보증형)"이 보이면 → 정확히 그대로
   - 절대 다른 상품명으로 바꾸지 말 것 (드림케어, 스마트케어 등 추측 금지)
   
2. 회사명: 이미지 상단 로고/CI/워터마크에서 확인
   - "KB" 로고가 보이면 → "KB라이프생명" 또는 "KB손해보험" 정확히 구분
   - KB라이프생명: 연금보험/종신보험/변액보험 등 생명보험 상품
   - KB손해보험: 자동차보험/화재보험/배상책임보험 등 손해보험 상품
   - 이미지에서 정확한 회사명이 안 보이면 상품 종류로 추론 (연금보험 → 생명보험사)
...
{
  "company": "정확한 보험회사명 (예: KB라이프생명, 삼성생명, 한화생명)",
  "product_name": "이미지에 인쇄된 상품명 그대로 (한 글자도 변경 금지)",
  "product_type": "연금보험/종신보험/암보험/어린이보험/교육보험/변액보험/저축보험/건강보험",
  "insured_age": "피보험자 나이 (숫자만, 예: 10)",
  "insured_gender": "남/여",
  "payment_period": "납입기간 숫자만 (예: 5)",
  "coverage_period": "보험기간 (예: 85세만기)",
  "monthly_premium": "월보험료 (예: 300,000원 또는 30만원)",
  "total_premium": "총납입금액 (예: 18,000,000원 또는 1,800만원)",
  "insurance_amount": "보험가입금액 (예: 1,800만원)",
  "surrender_values": [
    {"year": 5, "amount": "이미지 표기 그대로", "rate": "42.4%"},
    {"year": 10, "amount": "이미지 표기 그대로", "rate": "130.0%"}
  ],
  "interest_rate": "공시이율 X.X% / 최저보증이율 X.X% (이미지에 표기된 그대로)",
  "key_benefits": ["특약명 그대로"],
  "special_notes": "설계서 특이사항",
...
■ 필수 포함: "${analysisData.product_type || '연금보험'}" 또는 "${company}" 1개 이상
■ 사용자 상황(${writer.why}) 자연스럽게 반영
■ 금지: 가이드, 전략, 충격, 필독, 대박, 절대, 총정리
■ SEO 핵심: 네이버 사용자가 실제로 검색할 질문 + 핵심 숫자(환급률/월보험료 등)
...
【설계서 핵심 공유 (5~6줄)】  
- 월보험료/납입기간/총납입/해약환급금/환급률 정확히 인용
- 🚨🚨🚨 이익 금액 반드시 포함! 아래 문장을 본문에 그대로 넣으세요:
  "${bestProfitText || '(이익 계산 데이터 없음)'}"
- 공시이율/최저보증이율도 언급

【적금 비교 고민 (4~5줄)】
- 은행 적금 vs 이 보험 고민
- 중도해지 시 원금 손실 걱정
- 설계사는 좋다 하는데 본인은 판단 못하겠다는 불안

【질문 3개 (체크박스)】
- "✔️"로 시작, 구체적·전문적 질문 (단순 "좋나요?" 금지)
...
■ 질문자 상황 공감 + 격려
■ 🔴 이익 금액 "${bestProfitText}" 반드시 답변에 포함!
■ 설계서 숫자 기반 객관적 평가 (환급률 vs 은행적금 3~4% 비교)
■ 주의할 점 2~3가지 (중도해지 위험, 공시이율 변동, 유동성 등)
■ 다른 보험사 비교 추천 ("같은 ${analysisData.product_type || '보험상품'}이라도 보험사마다 환급률이 다르거든요")
■ "보험상담이필요하신분은 2~3개 보험사 설계서 비교 추천"
■ 따뜻한 마무리
...
- SEO: "${analysisData.product_type || '연금보험'}" 2~3회, "${company}" 1~2회
...
- 700~1000자`
      ...
      const answerResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `수천 명이 사용합니다. 시드=${seed}로 매번 완전히 다른 표현·문체·비유 사용. 당신은 "${expert.nick}" (${expert.career})으로, 네이버 보험 카페 인기 답변러. 🚨절대규칙: 1)상품명 "${productName}" 정확히 사용! 다른 이름 금지! 2)회사명 "${company}" 정확히! 3)설계서 숫자만 사용, 환각 금지! 4)🔴이익금액 "${bestProfitText}" 반드시 답변에 포함! SEO/C-RANK 키워드 자연 반복. 순수 텍스트 700~1000자.` },
...
[원글 제목] ${qaData.title || '보험 상품 문의'}
[원글 핵심] ${company} ${productName}, ${paymentPeriod}년납 월 ${monthlyPremium}
...
- 120~180자, "${analysisData.product_type || '연금보험'}" 키워드 포함
...
[상품] ${company} ${productName} (${analysisData.product_type || '보험상품'}), ${insuredAge}세, ${paymentPeriod}년납

해시태그 10개 생성 (네이버 SEO/C-RANK/AEO/GEO 최적화):

■ 구성:
- 상품 식별 (2~3개): #${company.replace(/\\s/g, '')} + 상품 관련
- 행동 유도 (2~3개): 보험상담/비교/리모델링 관련
- 니즈 키워드 (2~3개): 상품 목적 관련 (교육자금/노후/적금대안 등)
- 정보 키워드 (1~2개): 해약환급금/보험설계서 등

■ 규칙:
- #으로 시작, 띄어쓰기 없이
- 정확히 10개, 중복 절대 금지
- 회사명은 공식명칭 (예: #KB라이프생명, #삼성생명, #한화생명)
...
JSON: {"hashtags": ["#태그1","#태그2","#태그3","#태그4","#태그5","#태그6","#태그7","#태그8","#태그9","#태그10"]}`
      ...
      const hashtagResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4o',
          messages: [
            { role: 'system', content: `네이버 SEO/C-RANK/AEO/GEO 전문가. 시드=${seed}로 매번 다른 키워드 조합 생성. 중복 태그 금지. 실제 검색량 높은 키워드만.` },
