...
[🔴🔴🔴 절대 규칙 - 초보자 질문 스타일]
1. 🚨 설계서의 실제 담보명/특약명을 "그대로" 질문에 넣되, 초보자처럼 물어볼 것!
   - ❌ "고액치료비암진단비의 보장범위는?" (전문가 질문)
   - ❌ "특약이 뭐예요?" (너무 모호함 - 구체적 담보명 없음)
   - ✅ "여기 '고액치료비암진단비'라고 있는데 이게 뭐예요? 2000만원이면 충분한 건가요?" (초보자 + 구체적)
   - ✅ "'혈전용해치료비'라고 200만원 되어있는데 이게 필요한 건가요?" (초보자 + 실제 담보명 + 금액)
2. "~인데요", "~거든요", "~모르겠어요", "~뭐예요?" 같은 구어체 사용
3. 🚨🚨 각 질문에 반드시 설계서에 있는 실제 담보명 또는 금액을 1개 이상 인용!
   - 담보명 예: ${benefits || '특약명 확인 필요'}
   - 이 담보명들을 그대로 따옴표로 감싸서 질문에 넣으세요!
4. 너무 엉뚱하면 안 됨 → 설계서에서 볼 수 있는 내용 범위 내에서 질문
5. 하나의 질문이 카페 게시글 제목이 될 수 있는 수준의 구체성
6. 🚨 이미지에 없는 보험회사명/상품명을 절대 추측하지 마세요! 모르면 그냥 "보험" "이 보험"으로!

[질문 카테고리 반드시 포함]
- 3~4개: 🚨 실제 담보명을 인용한 질문 ("'혈전용해치료비'가 뭐예요?", "'뇌혈관질환진단비' 2000만원이면 많은 건가요?")
- 2~3개: 보험료/보장금액 관련 ("이 금액이 적은 건가요?", "담보마다 금액이 다른데 왜 그런가요?")
- 1~2개: 비교/필요성 질문 ("이 담보 다 필요한가요?", "빼도 되는 거 있나요?")
...
JSON:
{
  ...
  "reference_data": {
    ...
    "출처": "가입설계서"
      ...
      // V2026.37.109 - 전문가 닉네임 풀: 건강보험/연금보험 분리
